...
document.addEventListener('DOMContentLoaded', () => {
    ...
    productList.addEventListener('click', async (e) => {
        ...
        if (targetButton.classList.contains('baserow-btn')) {
            targetButton.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
            targetButton.disabled = true;
            try {
                const response = await fetch(`${API_BASE_URL}/api/send-to-baserow/${id}`, { method: 'POST', headers: getAuthHeaders() });
                if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.message || 'Back-end falhou ao enviar.'); }
                targetButton.innerHTML = '<i class="fas fa-check"></i>';
                targetButton.title = 'Enviado com Sucesso!';
            } catch (error) {
                console.error("Erro no envio para Baserow:", error);
                alert(`Falha ao enviar para o Baserow: ${error.message}`);
                targetButton.innerHTML = '<i class="fas fa-database"></i>';
